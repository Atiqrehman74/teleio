...
module.exports = async (req, res) => {
  ...
  try {
    const { property_id, checkin, checkout, currency = 'USD', recommendation_id } = req.query;
    if (!property_id) return res.status(400).json({ error: 'property_id is required' });
    if (!checkin || !checkout) return res.status(400).json({ error: 'checkin and checkout are required' });

    let path = `/resorts/api/v2/properties/availability?property_id=${encodeURIComponent(property_id)}&checkin=${encodeURIComponent(checkin)}&checkout=${encodeURIComponent(checkout)}&currency=${encodeURIComponent(currency)}`;
